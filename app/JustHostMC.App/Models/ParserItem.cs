...
public sealed class ParserItem : ScriptEntryItem
{
    public ParserItem(ParserInfo info, ILocalizer localizer)
        : base(
            ...
            info.Author,
            info.Version,
            ComposeDescription(info, localizer),
            info.Permissions,
            info.Granted,
            localizer)
    {
    ...
    private static string ComposeDescription(ParserInfo info, ILocalizer localizer)
    {
        if (info.Formats.Count == 0)
            return info.Description;
        var formats = localizer.Get("Parsers_Formats", ("formats", string.Join(", ", info.Formats)));
        return info.Description.Length == 0 ? formats : $"{info.Description}\n{formats}";
    }
}
