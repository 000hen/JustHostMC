...
public sealed class PlayerInventoryItemView
{
    ...
    public string Tooltip => HasItem
        ? $"{StyledName}\n{ItemId}\n{SlotName}"
        : SlotName;

    public static PlayerInventoryItemView Empty(int slot, string slotName) => new(slot, slotName);

    public static async Task<PlayerInventoryItemView> CreateAsync(PlayerInventoryItem item)
    {
