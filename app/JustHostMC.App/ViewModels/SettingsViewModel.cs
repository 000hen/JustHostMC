...
public sealed partial class SettingsViewModel : ObservableObject {
    ...
    public bool HasIncompleteInstallations =>
        IncompleteInstallationCount > 0;
    public string IncompleteInstallationDescription => _localizer.Get(
        "Settings_IncompleteInstallationsDescription",
    ...
    private async Task RefreshIncompleteInstallationsAsync() {
        try {
            ...
            var list = await daemon.Servers.ListAsync(
                new Empty(), deadline: DateTime.UtcNow.AddSeconds(30));
            var count = list.Servers.Count(server =>
    ...
    private async Task RemoveIncompleteInstallations() {
        RunOnUI(() => {
            IsBusy = true;
            StatusMessage = _localizer.Get(
                "Settings_RemovingIncompleteInstallations");
        });
        try {
            var daemon = await App.Current.DaemonReady;
            var list = await daemon.Servers.ListAsync(
                new Empty(), deadline: DateTime.UtcNow.AddSeconds(30));
            var incomplete = list.Servers.Where(server =>
                server.Status == ServerStatus.Installing).ToArray();
