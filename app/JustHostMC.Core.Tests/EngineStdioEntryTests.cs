...
public class EngineStdioEntryTests
{
    ...
    [InlineData("[INFO] grpc unary completed", EngineDiagnosticLevel.Information)]
    ...
    [InlineData("legacy untagged engine output", EngineDiagnosticLevel.Information)]
    public void Level_ParsesEngineSeverity(string message, EngineDiagnosticLevel expected)
    {
    ...
    [Theory]
    [InlineData("2026/07/07 14:25:37.123456 [INFO] grpc unary completed", "grpc unary completed")]
    [InlineData("2026/07/07 14:25:37.123456 [ERROR] registry write failed", "registry write failed")]
    [InlineData("MCMANAGER_READY", "MCMANAGER_READY")]
    public void DisplayMessage_RemovesDuplicateGoLogPrefix(string raw, string expected)
    {
        var entry = new EngineStdioEntry(
            1,
            DateTimeOffset.UtcNow,
