...
public sealed class ModFileItem
{
    ...
    /// <summary>Parsed display name, falling back to the jar filename.</summary>
    public string Title => HasMetadata && DisplayName.Length > 0 ? DisplayName : Name;

    /// <summary>Locale-neutral detail line: "version · authors" (either part optional).</summary>
    public string InfoLine => string.Join(" · ", new[] { Version, Authors }.Where(s => s.Length > 0));
    ...
    public Visibility FileNameVisibility =>
        HasMetadata && DisplayName.Length > 0 ? Visibility.Visible : Visibility.Collapsed;
