...
/// <summary>
/// Best-effort client-side parser for the permissions a Lua script declares in its
/// <c>meta.permissions</c> table, so the consent dialog can show them BEFORE import.
/// The engine re-parses and enforces the real declaration; this is UI-only.
/// </summary>
public static partial class LuaPermissions
{
    // Maps the lowercase script-facing names (see engine permissions.go) to enum kinds.
    ...
    [GeneratedRegex(@"\{\s*(?=[^}]*kind\s*=\s*[""']([a-z_]+)[""'])(?:[^}]*reason\s*=\s*[""']((?:\\.|[^""'\\])*)[""'])?[^}]*\}",
