...
/// <summary>
/// Application entry point. Owns the engine child-process lifecycle: it launches
/// the engine on startup, exposes the connected <see cref="DaemonClient"/> via
/// <see cref="DaemonReady"/>, and shuts the engine down when the window closes.
/// </summary>
public partial class App : Application
{
    ...
    private async Task StartEngineAsync()
    {
        try
        {
            ...
            var connection = await _engineHost.StartAsync().ConfigureAwait(false);
    ...
    /// <summary>
    /// Resolves the engine's data directory. When packaged (MSIX), all data lives
    /// under the package's local store so an uninstall removes it cleanly (PROMPT
    /// §8); when running unpackaged (dev/test), returns null so the engine uses its
