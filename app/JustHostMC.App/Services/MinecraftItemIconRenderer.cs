...
/// <summary>Interprets Minecraft item/model JSON and renders it from raw
/// resource-pack assets.</summary>
internal static class MinecraftItemIconRenderer {
    ...
    public static async Task<ImageSource?> RenderAsync(
        string itemId, IEnumerable<PlayerItemAsset> assetMessages) {
        if (!TryResourceId(itemId, "minecraft", out var itemNamespace,
                           out var itemPath))
            return null;

        var assets = new AssetSet();
        foreach (var message in assetMessages) await assets.AddAsync(message);
        ...
        foreach (var part in parts) {
            ...
            if (part.Special is not null)
                AddSpecialModel(quads, model, part.Special, part.Tints,
                                part.Transformations, assets);
            else if (model.Generated)
                AddGeneratedModel(quads, model, part.Tints,
                                  part.Transformations, assets);
            else
                AddBlockModel(quads, model, part.Tints, part.Transformations,
                              assets);
        }
        if (quads.Count == 0)
            return null;
    ...
    private static List<ItemPart> ResolveItemParts(string itemNamespace,
                                                   string itemPath,
                                                   AssetSet assets) {
    ...
    private static List<ItemPart> ResolveItemNode(JsonNode? node,
                                                  string defaultNamespace) {
        if (node is null)
            return [];
        if (node is JsonValue value &&
            value.TryGetValue<string>(out var directModel))
            return
                [new ItemPart(Qualify(directModel, "minecraft"), [], null, [])];
        ...
        switch (type) {
            case "minecraft:model": {
                ...
                result =
                    string.IsNullOrWhiteSpace(modelRef)
                        ? []
                        : [new ItemPart(Qualify(modelRef, "minecraft"),
                                        ReadTints(model["tints"]), null, [])];
                break;
            }
            case "minecraft:special": {
                ...
                result = string.IsNullOrWhiteSpace(baseModel)
                             ? []
                             : [new ItemPart(Qualify(baseModel, "minecraft"),
                                             ReadTints(model["tints"]),
            ...
            case "minecraft:composite": {
                ...
                if (model["models"] is JsonArray children)
                    foreach (var child in children)
                        result.AddRange(
                            ResolveItemNode(child, defaultNamespace));
                break;
            }
            case "minecraft:condition":
                result = ResolveItemNode(model["on_false"] ?? model["on_true"],
                                         defaultNamespace);
                break;
            case "minecraft:range_dispatch":
                result = ResolveItemNode(
                    model["fallback"] ?? FirstNestedModel(model),
                    defaultNamespace);
                break;
            case "minecraft:select":
                result = ResolveItemNode(SelectGuiModel(model) ??
                                             model["fallback"] ??
                                             FirstNestedModel(model),
                                         defaultNamespace);
                break;
            case "minecraft:empty":
                ...
                break;
            default:
                // Unknown extension types may still wrap a normal model.
                // Following their declared fallback is safer than item-name
                // special cases.
                result = ResolveItemNode(model["fallback"] ?? model["model"],
                                         defaultNamespace);
                break;
        }
    ...
    private static List<ItemPart> WithTransformation(List<ItemPart> parts,
                                                     JsonNode? node) {
        ...
        foreach (var key in new[] { "entries", "cases" }) {
            ...
            foreach (var entry in entries)
                if (entry is JsonObject entryObject &&
                    entryObject["model"] is JsonNode nested)
                    return nested;
        }
        return null;
    }
    ...
    private static JsonNode? SelectGuiModel(JsonObject model) {
        ...
        foreach (var entry in cases.OfType<JsonObject>()) {
            var matchesGui = entry["when"] switch {
                JsonValue value when value.TryGetValue<string>(
                    out var context) => context == "gui",
                JsonArray contexts   => contexts.Any(
                    context => context?.GetValue<string>() == "gui"),
                _ => false,
            };
            if (matchesGui)
                return entry["model"];
    ...
    private static Pixel? ReadColor(JsonNode? node) {
        ...
        if (node is JsonArray array && array.Count >= 3 &&
            array[0] is not null && array[1] is not null &&
            array[2] is not null) {
            return new Pixel(
    ...
    private static Pixel FromRgb(int rgb) => new((byte)(rgb >> 16),
                                                 (byte)(rgb >> 8), (byte)rgb,
                                                 255);

    private static ResolvedModel? ResolveModel(string reference,
                                               AssetSet assets,
                                               HashSet<string> visiting) {
        if (!TryResourceId(reference, "minecraft", out var modelNamespace,
                           out var modelPath))
            return null;
        ...
        if (!visiting.Add(id))
            return null;
        try {
            if (id is "minecraft:builtin/generated" or
                      "minecraft:builtin/entity")
                return new ResolvedModel {
                    Generated = id == "minecraft:builtin/generated"
                };
            ...
            try {
                document = JsonSerializer.Deserialize<ModelDocument>(
                    json, JsonOptions);
            } catch (JsonException) {
                return null;
            }
            ...
            var resolved =
                string.IsNullOrWhiteSpace(document.Parent)
                    ? new ResolvedModel()
                    : ResolveModel(Qualify(document.Parent, "minecraft"),
                                   assets, visiting) ??
                          new ResolvedModel();
            ...
            resolved.Generated |=
                document.Parent is
                "builtin/generated" or "minecraft:builtin/generated" or
                "item/generated" or "minecraft:item/generated";
            return resolved;
        } finally {
            visiting.Remove(id);
        }
    ...
    private static void AddGeneratedModel(
        List<RenderQuad> quads, ResolvedModel model,
        IReadOnlyList<Pixel?> tints,
        IReadOnlyList<ItemTransformation> transformations, AssetSet assets) {
        var transform = model.GuiTransform;
        for (var layer = 0;; layer++) {
            if (!model.Textures.TryGetValue($"layer{layer}",
                                            out var reference)) {
                if (layer == 0)
                    continue;
                break;
            ...
            var texture =
                assets.Texture(ResolveTexture(reference, model.Textures));
            if (texture is null)
                continue;
            var points =
                new[] { V(0, 16, 8), V(16, 16, 8), V(16, 0, 8), V(0, 0, 8) };
            var vertices = new RenderVertex[4];
            var uvs = new[] { new Vec2(0, 0), new Vec2(16, 0), new Vec2(16, 16),
                              new Vec2(0, 16) };
            for (var index = 0; index < 4; index++)
                vertices[index] =
                    new RenderVertex(ApplyGuiTransform(points[index], transform,
                                                       transformations),
        ...
        foreach (var element in model.Elements) {
            foreach (var (faceName, face) in element.Faces) {
                var texture = assets.Texture(
                    ResolveTexture(face.Texture, model.Textures));
                ...
                if (element.Rotation is not null)
                    normal = RotateAxis(normal, element.Rotation.Axis,
                ...
                if (turns > 0)
                    uvPoints =
                        Enumerable.Range(0, 4)
                            .Select(index => uvPoints[(index + turns) % 4])
                            .ToArray();
                ...
                for (var index = 0; index < 4; index++) {
                    var point =
                        element.Rotation is null
                            ? points[index]
                            : RotateElement(points[index], element.Rotation);
                    vertices[index] = new RenderVertex(
                        ApplyGuiTransform(point, transform, transformations),
                        uvPoints[index].X, uvPoints[index].Y);
                }
                var brightness =
                    !element.Shade ||
                            string.Equals(model.GuiLight, "front",
                                          StringComparison.OrdinalIgnoreCase)
                        ? 1
                        : NormalBrightness(normal);
                quads.Add(new RenderQuad(vertices, texture, brightness,
                                         Tint(tints, face.TintIndex)));
            }
        }
    }

    private static void AddSpecialModel(
        List<RenderQuad> quads, ResolvedModel baseModel, JsonObject special,
        IReadOnlyList<Pixel?> tints,
        IReadOnlyList<ItemTransformation> transformations, AssetSet assets) {
        ...
        if (!TryResourceId(textureRef ?? "", "minecraft",
                           out var textureNamespace, out var texturePath))
            return;
        var texture =
            assets.Texture($"{textureNamespace}:entity/chest/{texturePath}");
        if (texture is null)
            return;

        var model = baseModel.Clone();
        model.Textures["special"] =
            $"{textureNamespace}:entity/chest/{texturePath}";
        model.Elements = [
            Box([1, 1, 1], [15, 11, 15], [10.5, 8.25, 14, 10.75],
                [3.5, 8.25, 7, 10.75], [0, 8.25, 3.5, 10.75],
                [7, 8.25, 10.5, 10.75], [3.5, 4.75, 7, 8.25],
                [7, 4.75, 10.5, 8.25]),
            Box([1, 11, 1], [15, 16, 15], [10.5, 3.5, 14, 4.75],
                [3.5, 3.5, 7, 4.75], [0, 3.5, 3.5, 4.75], [7, 3.5, 10.5, 4.75],
            ...
            new ModelElement {
                ...
                Faces =
                    new(StringComparer.OrdinalIgnoreCase) {
                                                           ["south"] =
                            new() { Texture = "#special", Uv = [0, 0, 0.5, 1] },
                                                           },
        },
        ];
    ...
    private static void AddShieldModel(
        List<RenderQuad> quads, ResolvedModel baseModel,
        IReadOnlyList<Pixel?> tints,
        IReadOnlyList<ItemTransformation> transformations, AssetSet assets) {
        ...
        model.Elements = [
            Box([-6, -11, 1], [6, 11, 2], Quarter([14, 1, 26, 23]),
                Quarter([1, 1, 13, 23]), Quarter([0, 1, 1, 23]),
                Quarter([13, 1, 14, 23]), Quarter([13, 0, 25, 1]),
                Quarter([1, 0, 13, 1])),
            Box([-1, -3, -5], [1, 3, 1], Quarter([40, 6, 42, 12]),
                Quarter([32, 6, 34, 12]), Quarter([26, 6, 32, 12]),
                Quarter([34, 6, 40, 12]), Quarter([34, 0, 36, 6]),
                Quarter([32, 0, 34, 6])),
        ];
        AddBlockModel(quads, model, tints,
                      WithoutShieldDefaultFlip(transformations), assets);
    }
    ...
    private static ModelElement Box(double[] from, double[] to, double[] north,
                                    double[] south, double[] west,
                                    double[] east, double[] up,
                                    double[] down) => new() {
        From = from,
        To   = to,
        Faces =
    ...
    private static byte[] Rasterize(IReadOnlyList<RenderQuad> quads) {
        ...
        var depth =
            Enumerable.Repeat(double.NegativeInfinity, Size* Size).ToArray();
        foreach (var quad in quads) {
            var vertices =
                quad.Vertices
                    .Select(vertex => vertex with {
                        Point = V(center + vertex.Point.X * pixelsPerModelUnit,
                                  center - vertex.Point.Y * pixelsPerModelUnit,
                                  vertex.Point.Z),
                    })
                    .ToArray();
            RasterizeTriangle(output, depth, vertices[0], vertices[1],
                              vertices[2], quad);
            RasterizeTriangle(output, depth, vertices[0], vertices[2],
                              vertices[3], quad);
        }
        return output;
    }

    private static void RasterizeTriangle(byte[] output, double[] depth,
                                          RenderVertex a, RenderVertex b,
                                          RenderVertex c, RenderQuad quad) {
        var minX = Math.Max(0, (int)Math.Floor(Math.Min(
                                   a.Point.X, Math.Min(b.Point.X, c.Point.X))));
        var maxX =
            Math.Min(Size - 1, (int)Math.Ceiling(Math.Max(
                                   a.Point.X, Math.Max(b.Point.X, c.Point.X))));
        var minY = Math.Max(0, (int)Math.Floor(Math.Min(
                                   a.Point.Y, Math.Min(b.Point.Y, c.Point.Y))));
        var maxY =
            Math.Min(Size - 1, (int)Math.Ceiling(Math.Max(
                                   a.Point.Y, Math.Max(b.Point.Y, c.Point.Y))));
        ...
        for (var y = minY; y <= maxY; y++)
            for (var x = minX; x <= maxX; x++) {
                ...
                var w1 = ((b.Point.Y - c.Point.Y) * (px - c.Point.X) +
                          (c.Point.X - b.Point.X) * (py - c.Point.Y)) /
                         denominator;
                var w2 = ((c.Point.Y - a.Point.Y) * (px - c.Point.X) +
                          (a.Point.X - c.Point.X) * (py - c.Point.Y)) /
                         denominator;
                var w3 = 1 - w1 - w2;
                ...
                var color = quad.Texture.Pixel(
                    Math.Clamp((int)(u / 16 * quad.Texture.FrameSize), 0,
                               quad.Texture.FrameSize - 1),
                    Math.Clamp((int)(v / 16 * quad.Texture.FrameSize), 0,
                               quad.Texture.FrameSize - 1));
                if (color.A == 0)
                    continue;
                if (quad.Tint is Pixel tint)
                    color = new Pixel((byte)(color.R * tint.R / 255),
                                      (byte)(color.G * tint.G / 255),
                                      (byte)(color.B * tint.B / 255), color.A);
                color = new Pixel((byte)(color.R * quad.Brightness),
                                  (byte)(color.G * quad.Brightness),
    ...
    private static Vec3[]? FacePoints(double[] from, double[] to,
                                      string face) => face switch {
        "north" => [V(from[0], to[1], from[2]), V(to[0], to[1], from[2]),
                    V(to[0], from[1], from[2]), V(from[0], from[1], from[2])],
        "south" => [V(to[0], to[1], to[2]), V(from[0], to[1], to[2]),
                    V(from[0], from[1], to[2]), V(to[0], from[1], to[2])],
        "west"  => [V(from[0], to[1], to[2]), V(from[0], to[1], from[2]),
                    V(from[0], from[1], from[2]), V(from[0], from[1], to[2])],
        "east"  => [V(to[0], to[1], from[2]), V(to[0], to[1], to[2]),
                    V(to[0], from[1], to[2]), V(to[0], from[1], from[2])],
        "up"    => [V(from[0], to[1], from[2]), V(from[0], to[1], to[2]),
                    V(to[0], to[1], to[2]), V(to[0], to[1], from[2])],
        "down"  => [V(from[0], from[1], to[2]), V(from[0], from[1], from[2]),
                    V(to[0], from[1], from[2]), V(to[0], from[1], to[2])],
        _       => null,
    };

    private static double[] DefaultFaceUv(double[] from, double[] to,
                                          string face) => face switch {
    ...
    private static Vec3 RotateElement(Vec3 point, ModelRotation rotation) {
        var origin =
            V(rotation.Origin[0], rotation.Origin[1], rotation.Origin[2]);
    ...
    private static Vec3 ApplyGuiTransform(
        Vec3 point, ModelTransform transform,
        IReadOnlyList<ItemTransformation> transformations) {
        point -= V(8, 8, 8);
        foreach (var itemTransform in transformations)
            point = ApplyItemTransform(point, itemTransform);
        point =
            V(point.X * transform.ScaleAt(0), point.Y * transform.ScaleAt(1),
              point.Z * transform.ScaleAt(2));
        ...
        return point + V(transform.TranslationAt(0), transform.TranslationAt(1),
                         transform.TranslationAt(2));
    }

    private static Vec3 ApplyItemTransform(Vec3 point,
    ...
    private static Vec3 ApplyItemNormal(
        Vec3 normal, IReadOnlyList<ItemTransformation> transformations) {
        foreach (var transform in transformations) {
            normal = RotateQuaternion(normal, transform.RightRotation);
            normal = V(NormalScale(normal.X, transform.ScaleAt(0)),
    ...
    private static Vec3 Cross(Vec3 left, Vec3 right) => V(
        left.Y * right.Z - left.Z * right.Y, left.Z* right.X - left.X * right.Z,
        left.X* right.Y - left.Y * right.X);

    private static Vec3 ApplyGuiRotation(Vec3 vector,
    ...
    private static Vec3 RotateAxis(Vec3 point, string axis, double degrees) {
        ...
        return axis switch {
            "x" => V(point.X, point.Y * cosine - point.Z * sine,
                     point.Y * sine + point.Z * cosine),
            "y" => V(point.X * cosine + point.Z * sine, point.Y,
                     -point.X * sine + point.Z * cosine),
            "z" => V(point.X * cosine - point.Y * sine,
    ...
    private static string ResolveTexture(
        string reference, IReadOnlyDictionary<string, string> textures) {
    ...
    private static Pixel? Tint(IReadOnlyList<Pixel?> tints,
                               int index) => index >= 0 && index < tints.Count
                                                 ? tints[index]
                                                 : null;

    private static Vec3 FaceNormal(string face) => face switch {
        "north" => V(0, 0, -1), "south" => V(0, 0, 1), "west" => V(-1, 0, 0),
        "east" => V(1, 0, 0),   "up" => V(0, 1, 0),    "down" => V(0, -1, 0),
        _ => V(0, 0, 0),
    };
    ...
    private static void Blend(byte[] target, int index, Pixel source) {
        ...
        target[index + 1] =
            (byte)(source.G * alpha + target[index + 1] * (1 - alpha));
        target[index + 2] =
            (byte)(source.R * alpha + target[index + 2] * (1 - alpha));
        target[index + 3] =
            (byte)Math.Min(255, source.A + target[index + 3] * (1 - alpha));
    }

    private static string Qualify(string value, string defaultNamespace) =>
        value.Contains(':') ? value : $"{defaultNamespace}:{value}";

    private static bool TryResourceId(string value, string defaultNamespace,
                                      out string resourceNamespace,
                                      out string path) {
        value         = value.Trim().TrimStart('/');
        var separator = value.IndexOf(':');
        resourceNamespace =
            separator >= 0 ? value[..separator] : defaultNamespace;
    ...
    private sealed class AssetSet {
        ...
        public Dictionary<string, string> Models { get; } =
            new(StringComparer.OrdinalIgnoreCase);
        private Dictionary<string, TextureData> Textures { get; } =
            new(StringComparer.OrdinalIgnoreCase);

        public async Task AddAsync(PlayerItemAsset asset) {
            var path =
                asset.Path.Replace('\\', '/').TrimStart('/').ToLowerInvariant();
            if (path.EndsWith(".json", StringComparison.Ordinal)) {
                ...
                var texture =
                    await DecodeTextureAsync(asset.Data.ToByteArray());
                if (texture is not null)
                    Textures[id] = texture;
            }
        }

        public TextureData? Texture(string id) => Textures.GetValueOrDefault(
        ...
        public string? FindTextureId(params string[] preferredIds) {
            ...
            return Textures.Keys.FirstOrDefault(
                id => id.EndsWith('/' + fileName,
                                  StringComparison.OrdinalIgnoreCase));
        }

        private static bool TryPathId(string path, string kind, string suffix,
                                      out string id) {
            id = "";
            var segments =
                path.Split('/', StringSplitOptions.RemoveEmptyEntries);
            if (segments.Length < 4 || segments[0] != "assets" ||
                segments[2] != kind || !path.EndsWith(suffix))
                return false;
    ...
    private static async Task<TextureData?> DecodeTextureAsync(byte[] png) {
        ...
        try {
            ...
            return new TextureData((int)decoder.PixelWidth,
    ...
    private sealed class ResolvedModel {
        ...
        public ModelTransform GuiTransform =>
            Display.GetValueOrDefault("gui") ?? new();
        ...
        public ResolvedModel Clone() => new() {
            ...
            Elements = Elements,
            Display  = new Dictionary<string, ModelTransform>(
                Display, StringComparer.OrdinalIgnoreCase),
        };
    }
    ...
    private sealed class ModelTransform {
        ...
        public double RotationAt(int index) => Rotation is { Length : > 2 }
                                                   ? Rotation[index]
                                                   : 0;
        public double TranslationAt(int index) =>
            Translation is { Length : > 2 }
                ? Math.Clamp(Translation[index], -80, 80)
                : 0;
        public double ScaleAt(int index) => Scale is { Length : > 2 }
                                                ? Math.Min(Scale[index], 4)
                                                : 1;
    }
    ...
    private sealed class ItemTransformation {
        ...
        public double ScaleAt(int index)       => Scale is { Length : > 2 }
                                                      ? Scale[index]
                                                      : 1;
        public double TranslationAt(int index) => Translation
                                                      is { Length : > 2 }
                                                      ? Translation[index]
                                                      : 0;
        ...
        private static bool IsIdentityRotation(double[]? value) =>
            value is null ||
            (value.Length == 4 && Math.Abs(value[0]) <= Epsilon &&
             Math.Abs(value[1]) <= Epsilon && Math.Abs(value[2]) <= Epsilon &&
    ...
    private sealed class TextureData(int width, int height, byte[] pixels) {
        ...
        public Pixel Pixel(int x, int y) {
            var index = (Math.Clamp(y, 0, height - 1) * width +
                         Math.Clamp(x, 0, width - 1)) *
                        4;
            return new Pixel(pixels[index], pixels[index + 1],
    ...
    private readonly record struct Vec3(double X, double Y, double Z) {
        public static Vec3 operator +(Vec3 left, Vec3 right) =>
            V(left.X + right.X, left.Y + right.Y, left.Z + right.Z);
        public static Vec3 operator -(Vec3 left, Vec3 right) =>
            V(left.X - right.X, left.Y - right.Y, left.Z - right.Z);
        public static Vec3 operator*(Vec3 value, double scalar) =>
            V(value.X * scalar, value.Y* scalar, value.Z* scalar);
    }
    private readonly record struct RenderVertex(Vec3 Point, double U, double V);
    private sealed record RenderQuad(RenderVertex[] Vertices,
                                     TextureData Texture, double Brightness,
                                     Pixel? Tint);
    private sealed record ItemPart(string Model, List<Pixel?> Tints,
                                   JsonObject? Special,
