...
public class DataDirTests
{
    ...
    {
        var dataDir = Path.Combine(Path.GetTempPath(), "jhmc-datadir-" + Guid.NewGuid().ToString("N"));
        try
        {
            ...
            // The engine creates its SQLite registry under the data dir's base before
            // it reports a port, so by now the file must exist there.
            Assert.True(
