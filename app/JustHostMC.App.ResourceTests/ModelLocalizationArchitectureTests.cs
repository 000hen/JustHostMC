...
public sealed class ModelLocalizationArchitectureTests {
    ...
    public void BanEntryFiniteLabelsBelongToItsXamlTemplate() {
        var model = File.ReadAllText(
            RepositoryLayout.AppPath("Models", "BanEntryItem.cs"));
        var xaml = File.ReadAllText(
            RepositoryLayout.AppPath("Views", "BanListDialog.xaml"));
        ...
        Assert.DoesNotContain("BanList_NoReason", model,
                              StringComparison.Ordinal);
        Assert.Contains("BanListEntryIpTypeText", xaml,
                        StringComparison.Ordinal);
    ...
    public void ParserDiagnosticsAreNotFormattedIntoUserFacingModErrors() {
        var model = File.ReadAllText(
            RepositoryLayout.AppPath("Models", "ModFileItem.cs"));
