...
/// <summary>Streams ~1 Hz resource samples from the MetricsService and exposes them
/// as bounded normalized series (for sparklines) plus current-value readouts.</summary>
public sealed partial class MetricsViewModel : ObservableObject, IAsyncDisposable
{
    ...
    private async Task ReadLoopAsync(JustHostMC.Core.DaemonClient daemon, CancellationToken token)
    {
        try
        {
            using var call = daemon.Metrics.Watch(new ServerId { Id = _serverId }, cancellationToken: token);
            await foreach (var sample in call.ResponseStream.ReadAllAsync(token).ConfigureAwait(false))
            {
                var snapshot = sample;
    ...
    private void Apply(ResourceSample s)
    {
        ...
        {
            memNormalized = Math.Clamp(s.MemoryBytes / (double)s.MemoryLimitBytes, 0, 1);
        ...
        if (s.NetworkAvailable)
        {
            ...
            NetText = $"↓ {Kb(s.NetRxBytesPerSec)}  ↑ {Kb(s.NetTxBytesPerSec)} /s";
        }
    }
