...
/// <summary>Collects server fields for either a create or edit operation.</summary>
public sealed partial class ServerDialog : UserControl
{
    ...
    public ServerDialog(MainViewModel viewModel, ServerDialogMode mode, ServerItem? server = null)
    {
        if (mode == ServerDialogMode.Edit && server is null)
            throw new ArgumentNullException(nameof(server), "Edit mode requires a server.");
        if (mode == ServerDialogMode.Create && server is not null)
            throw new ArgumentException("Create mode does not accept an existing server.", nameof(server));
    ...
    private async Task LoadVersionsAsync(string providerId, string? selectedVersion = null)
    {
        ...
        try
        {
            var versions = (await _viewModel.GetVersionsAsync(providerId)).ToList();
            if (selectedVersion is not null && !versions.Contains(selectedVersion))
    ...
    public UpdateServerRequest BuildUpdateRequest()
    {
        ...
        var version = server.CanEditLaunchSettings && VersionBox.SelectedItem is string selectedVersion
            ? selectedVersion
            : server.McVersion;
        var port = server.CanEditLaunchSettings && !double.IsNaN(PortBox.Value)
            ? (int)PortBox.Value
            : server.Port;
        var memoryMb = server.CanEditLaunchSettings && !double.IsNaN(MemoryBox.Value)
            ? (int)MemoryBox.Value
            : server.MemoryMb;
    ...
    private void EnsureMode(ServerDialogMode expectedMode)
    {
        if (Mode != expectedMode)
            throw new InvalidOperationException($"Cannot build a {expectedMode} request in {Mode} mode.");
    }
}
