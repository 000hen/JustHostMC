...
internal static class EngineFixture {
    /// <summary>
    /// Resolves the engine executable copied next to the test assembly. If it
    /// is missing, the error explains how to produce it so the failure is
    /// actionable.
    /// </summary>
    ...
    public static EngineHost NewHost(Action<string>? onDiagnostic = null) =>
        new(new EngineHostOptions {
            ...
            DataDir    = Path.Combine(Path.GetTempPath(), "JustHostMC.Tests",
