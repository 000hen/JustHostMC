...
public sealed record ShopContext(
    ...
    string Loader,
    ModKind Kind,
    Func<IReadOnlyCollection<string>> InstalledFileNames,
    Action OnInstalled);
...
public sealed class ShopVersionItem
{
    ...
    {
        Version = version;
        RequiredDependencies = version.Dependencies.Where(d => d.Required).ToArray();
    }

    public ShopVersion Version { get; }
    public IReadOnlyList<ShopDependency> RequiredDependencies { get; }
    public string Name => Version.Name.Length > 0 ? Version.Name : Version.VersionNumber;
    ...
    public Visibility ChannelVisibility => ChannelText.Length == 0 ? Visibility.Collapsed : Visibility.Visible;
    public string GameVersionsText => string.Join(", ", Version.GameVersions.Take(6))
