...
public sealed class ProviderCatalog {
    ...
    public ProviderCatalog(Func<Task<IReadOnlyList<ProviderInfo>>> fetch) =>
        _fetch = fetch;

    /// <summary>Raised on the loading thread once the catalog is first
    /// populated.</summary>
    public event Action? Loaded;

    /// <summary>True once the catalog has been populated at least
    /// once.</summary>
    public bool IsLoaded { get; private set; }

    /// <summary>Loads (once) and returns the provider list; subsequent calls
    /// reuse the cache.</summary>
    ...
    private async Task<IReadOnlyList<ProviderInfo>> LoadAsync() {
        ...
        lock (_gate) {
            _byId    = providers.Where(p => !string.IsNullOrEmpty(p.Id))
                           .GroupBy(p => p.Id)
    ...
    /// <summary>Returns the cached provider for an id, or null if not yet
    /// loaded / unknown.</summary>
    public ProviderInfo? Find(string id) {
        if (string.IsNullOrEmpty(id))
            return null;
        lock (_gate) return _byId.TryGetValue(id, out var info) ? info : null;
    }

    /// <summary>The provider's friendly name, or null when
    /// unresolved.</summary>
    ...
    /// <summary>
    /// The provider's mod layout ("plugins" | "mods" | "none"), or null when
    /// the catalog has not yet loaded the given id (UI should assume supported
    /// until known).
    /// </summary>
