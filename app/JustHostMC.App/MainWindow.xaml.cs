...
public sealed partial class MainWindow : Window {
    ...
    public MainWindow() {
        ...
        InitializeComponent();
        _paneFooterVisibilityCallbackToken = PaneFooterGrid.RegisterPropertyChangedCallback(
            UIElement.VisibilityProperty,
            OnPaneFooterVisibilityChanged);
        ...
        Shell.HomeRequested += () => Nav.SelectedItem = NavigationDestination.Home;
    ...
    private void OnServersChanged(object? sender, NotifyCollectionChangedEventArgs e) {
        ...
        if (e.Action == NotifyCollectionChangedAction.Reset) {
            var live = Shell.Main.Servers.Select(server => server.Id).ToHashSet();
            foreach (var server in _trackedServers.Values.Where(server => !live.Contains(server.Id)).ToList()) {
                UntrackServer(server);
                _ = Shell.EvictServerCacheAsync(server.Id);
            }
    ...
    private void OnPaneFooterVisibilityChanged(DependencyObject sender, DependencyProperty property) {
        // NavigationView does not recalculate its menu ScrollViewer's cached MaxHeight when
        // PaneFooter content changes visibility. Wait for the footer's new size, then nudge
        // the PaneFooter property to invoke NavigationView's own pane-layout update path.
    ...
    private void MarkServerStateChange(ServerItem server) {
        ...
        server.HasUnreadStateChange = !ReferenceEquals(Nav.SelectedItem, server);
    }
    ...
    private void ShowNextServerTip() {
        ...
        while (_pendingServerTips.TryDequeue(out var notification)) {
            ...
            Nav.UpdateLayout();
            if (Nav.ContainerFromMenuItem(notification.Server) is not NavigationViewItem target)
                continue;
    ...
    private void OnContentFrameNavigated(object sender, NavigationEventArgs args) {
        object? item = args.SourcePageType switch {
            var pageType when pageType == typeof(HomePage) => NavigationDestination.Home,
            var pageType when pageType == typeof(ScriptsPage) => NavigationDestination.Scripts,
            var pageType when pageType == typeof(SettingsPage) => NavigationDestination.Settings,
            var pageType when pageType == typeof(ServerPage)
                && args.Parameter is ServerPageArgs serverArgs
                && _trackedServers.TryGetValue(serverArgs.Server.Id, out var serverItem) => serverItem,
            _ => null,
        };
